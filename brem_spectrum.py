...
## bremsstrhlung spectrum ##
  ...
  qi     = q0*79
  mi     = m0*1836*189
  en_photon = np.linspace(0.1,500.1,1000)*m0*v0**2
...
#  plt.yscale('log')
  ...
  qi     = q0*79
  mi     = m0*1836*189
  en_photon = np.linspace(1e-4,0.5,5000)*gg*m0*v0**2
...
#  plt.xscale('log')
  ...
  qi     = q0*79
  mi     = m0*1836*189
  gg     = np.linspace(1,1001,1001)
  dN_dl  = np.zeros([1001,5001])
  for i in range(1001):
      en_photon = np.linspace(1e-4,0.5,5001)*gg[i]*m0*v0**2
      for j in range(5001):
...
#  plt.xscale('log')
  ...
  fig.savefig('./brem_spectrum.png',format='png',dpi=160)
  plt.close("all")
